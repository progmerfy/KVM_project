...
def api_create_vm(req: VMCreateRequest, request: Request, current_user: dict = Depends(get_current_user)):
    ...
    create_audit_log(current_user["id"], current_user["username"], "create", "vm", req.name, ip_address=request.client.host if request.client else None)
    return JSONResponse(
        status_code=202,
...
def api_start_vm(req: VMActionRequest, request: Request, current_user: dict = Depends(get_current_user)):
    if not authorize_vm_access(req.name, current_user):
        create_audit_log(current_user["id"], current_user["username"], "start", "vm", req.name, success=False, ip_address=request.client.host if request.client else None)
        raise ServiceError("Access denied", code="FORBIDDEN", http_status=403)
    start_vm(req)
    create_audit_log(current_user["id"], current_user["username"], "start", "vm", req.name, ip_address=request.client.host if request.client else None)
    return {"status": "ok"}
...
def api_stop_vm(req: VMActionRequest, request: Request, current_user: dict = Depends(get_current_user)):
    if not authorize_vm_access(req.name, current_user):
        create_audit_log(current_user["id"], current_user["username"], "stop", "vm", req.name, success=False, ip_address=request.client.host if request.client else None)
        raise ServiceError("Access denied", code="FORBIDDEN", http_status=403)
    stop_vm(req)
    create_audit_log(current_user["id"], current_user["username"], "stop", "vm", req.name, ip_address=request.client.host if request.client else None)
    return {"status": "ok"}
...
def api_delete_vm(req: VMActionRequest, request: Request, current_user: dict = Depends(get_current_user)):
    if not authorize_vm_access(req.name, current_user):
        create_audit_log(current_user["id"], current_user["username"], "delete", "vm", req.name, success=False, ip_address=request.client.host if request.client else None)
        raise ServiceError("Access denied", code="FORBIDDEN", http_status=403)
    delete_vm(req)
    create_audit_log(current_user["id"], current_user["username"], "delete", "vm", req.name, ip_address=request.client.host if request.client else None)
    return {"status": "ok"}
...
def api_snapshot_create(name: str, snap_name: str, request: Request, current_user: dict = Depends(get_current_user), host_uri: str = None):
    if not authorize_vm_access(name, current_user):
        create_audit_log(current_user["id"], current_user["username"], "snapshot_create", "vm", name, success=False, ip_address=request.client.host if request.client else None)
        raise ServiceError("Access denied", code="FORBIDDEN", http_status=403)
    result = snapshot_create(name, snap_name, host_uri)
    create_audit_log(current_user["id"], current_user["username"], "snapshot_create", "vm", name, details=f"snapshot={snap_name}", ip_address=request.client.host if request.client else None)
...
def api_snapshot_delete(name: str, snap_name: str, request: Request, current_user: dict = Depends(get_current_user), host_uri: str = None):
    if not authorize_vm_access(name, current_user):
        create_audit_log(current_user["id"], current_user["username"], "snapshot_delete", "vm", name, success=False, ip_address=request.client.host if request.client else None)
        raise ServiceError("Access denied", code="FORBIDDEN", http_status=403)
    result = snapshot_delete(name, snap_name, host_uri)
    create_audit_log(current_user["id"], current_user["username"], "snapshot_delete", "vm", name, details=f"snapshot={snap_name}", ip_address=request.client.host if request.client else None)
...
def api_clone_vm(req: VMCloneRequest, request: Request, current_user: dict = Depends(get_current_user)):
    if not authorize_vm_access(req.name, current_user):
        create_audit_log(current_user["id"], current_user["username"], "clone", "vm", req.name, success=False, ip_address=request.client.host if request.client else None)
        raise ServiceError("Access denied", code="FORBIDDEN", http_status=403)
    result = clone_vm(req.name, req.new_name, req.host_uri, owner_id=current_user["id"])
    create_audit_log(current_user["id"], current_user["username"], "clone", "vm", req.new_name, details=f"source={req.name}", ip_address=request.client.host if request.client else None)
    return {"status": "ok", "vm": result}
...
def api_backup_vm(req: VMBackupRequest, request: Request, current_user: dict = Depends(get_current_user)):
    if not authorize_vm_access(req.name, current_user):
        create_audit_log(current_user["id"], current_user["username"], "backup", "vm", req.name, success=False, ip_address=request.client.host if request.client else None)
        raise ServiceError("Access denied", code="FORBIDDEN", http_status=403)
    result = backup_vm(req.name, req.host_uri)
    create_audit_log(current_user["id"], current_user["username"], "backup", "vm", req.name, details=f"backup_dir={result.get('backup_dir')}", ip_address=request.client.host if request.client else None)
...
def api_delete_backup(backup_dir: str, request: Request, current_user: dict = Depends(get_current_user)):
    if not current_user.get("is_admin"):
        create_audit_log(current_user["id"], current_user["username"], "backup_delete", "vm", "unknown", success=False, ip_address=request.client.host if request.client else None)
        raise ServiceError("Access denied", code="FORBIDDEN", http_status=403)
    delete_backup(backup_dir)
    create_audit_log(current_user["id"], current_user["username"], "backup_delete", "vm", backup_dir, ip_address=request.client.host if request.client else None)
...
def api_create_backup_schedule(req: BackupScheduleCreate, request: Request, current_user: dict = Depends(get_current_user)):
    if not current_user.get("is_admin"):
        create_audit_log(current_user["id"], current_user["username"], "schedule_create", "backup", req.vm_name, success=False, ip_address=request.client.host if request.client else None)
        raise ServiceError("Access denied", code="FORBIDDEN", http_status=403)
    try:
    ...
    try:
        ...
        create_audit_log(current_user["id"], current_user["username"], "schedule_create", "backup", req.vm_name, details=f"cron={req.cron_expression},retention={req.retention}", ip_address=request.client.host if request.client else None)
...
def api_update_backup_schedule(schedule_id: int, req: BackupScheduleUpdate, request: Request, current_user: dict = Depends(get_current_user)):
    if not current_user.get("is_admin"):
        create_audit_log(current_user["id"], current_user["username"], "schedule_update", "backup", str(schedule_id), success=False, ip_address=request.client.host if request.client else None)
        raise ServiceError("Access denied", code="FORBIDDEN", http_status=403)
    ...
    create_audit_log(current_user["id"], current_user["username"], "schedule_update", "backup", sched["vm_name"], details=f"schedule_id={schedule_id}", ip_address=request.client.host if request.client else None)
    return {"status": "ok", "schedule": sched}
...
def api_delete_backup_schedule(schedule_id: int, request: Request, current_user: dict = Depends(get_current_user)):
    if not current_user.get("is_admin"):
        create_audit_log(current_user["id"], current_user["username"], "schedule_delete", "backup", str(schedule_id), success=False, ip_address=request.client.host if request.client else None)
        raise ServiceError("Access denied", code="FORBIDDEN", http_status=403)
    sched = get_backup_schedule(schedule_id)
    if not sched:
        raise ServiceError("Schedule not found", code="NOT_FOUND", http_status=404)
    delete_backup_schedule(schedule_id)
    create_audit_log(current_user["id"], current_user["username"], "schedule_delete", "backup", sched["vm_name"], details=f"schedule_id={schedule_id}", ip_address=request.client.host if request.client else None)
